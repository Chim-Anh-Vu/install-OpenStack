...
GRANT ALL PRIVILEGES ON placement.* TO 'nova'@'localhost' IDENTIFIED BY '$NOVA_DBPASS';
GRANT ALL PRIVILEGES ON placement.* TO 'nova'@'%' IDENTIFIED BY '$NOVA_DBPASS';
...
nova_install () {
	...
	sleep 3
	apt install nova-api nova-conductor nova-novncproxy nova-scheduler nova-placement-api -y
}
...
nova_config () {
	...
	ops_add $novafile placement_database connection mysql+pymysql://nova:$PLACEMENT_PASS@$HOST_CTL/placement
...
#	ops_add $novafile keystone_authtoken auth_uri http://$HOST_CTL:5000
	ops_add $novafile keystone_authtoken auth_url http://$HOST_CTL:5000/v3
